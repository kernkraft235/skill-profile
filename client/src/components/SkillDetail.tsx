...
const SkillDetail = ({ 
  ...
  const { data: category, isLoading: categoryLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest('GET', `/api/skill-categories/${categoryId}`);
  ...
  const { data: subcategories } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest('GET', `/api/skill-categories/parent/${categoryId}`);
      const data = await res.json();
      return data as SkillCategory[];
    }
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{category.name} Skills</h1>
